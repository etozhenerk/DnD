...
PDF,AUDIT_PATH=output_paths('golovach-lena')
UI=ROOT/'assets/concepts/campaigns/penisuela/ui'
hero=next(x for x in json.loads((ROOT/'content/characters.json').read_text()) if x['id']=='golovach-lena')
gameplay=json.loads((ROOT/'content/campaigns/penisuela-gallery-gameplay.json').read_text())
actions={x['sourceId']:x for x in gameplay['combatActions'] if x['characterId']=='golovach-lena' and not x['id'].endswith('break-mirror-angle')}
...
c.setTitle('Головач Лена · Информационная карточка')
...
art(UI/'hero-tokens/golovach-lena.png',px,py,ps,ps)
c.restoreState()
header_center=382
...
skill_bodies={
 'buldak-breath':'До двух целей в линию: <b>Ловкость DC 13</b>. Провал: <b>2d6+3 огнём</b> и ещё <b>1d4</b> в начале следующего хода цели. Успех: половина основного урона.',
 'forgot-clothes':'Каждый полученный холодный удар даёт <b>+2 урона следующей огненной атаке</b>, максимум <b>+4</b>.',
 'name-story':'Союзник может перебросить <b>d20 атаки или спасброска</b>, увидев результат. Заряд сохраняется до переброса или конца боя.',
 'creative-crisis':'Брось <b>1d6</b>:<br/><b>1:</b> +2 AC до следующего хода.<br/><b>2-3:</b> +2 к атаке, +1d4 урона.<br/><b>4-5:</b> преимущество, +1d4 урона.<br/><b>6:</b> преимущество, крит на 19-20.<br/>Результаты 2-6: следующая атака.',
 'video-surveillance':'Раскрывает <b>следующее действие и цель врага</b>. Его следующая атака перебрасывается: берётся <b>худший результат</b>.'
}
item_bodies={
 'buldak-noodles':'Восстанавливаешь <b>15 HP</b> и получаешь <b>+2 к следующей атаке</b>. Всего две порции.',
 'fire-fists':'<b>Атака +6, урон 1d10+3 огнём.</b> С мастером: против ледяных существ ещё <b>+2 к броску атаки</b>.',
 'dragonborn-armour':'<b>+2 AC против ближних атак.</b> Получаемый холодный урон уменьшается на <b>1</b>.',
 'sleep-scroll':'Пропускаешь действие и восстанавливаешь себе <b>1d8 HP</b>.',
 'camera-pendant':'<b>+4 к Интеллекту для поиска секретов.</b> Через «Видеонаблюдение» обнаруживает скрытые объекты, тайные проходы и необычные детали.'
}
def meta(a):
...
for a in hero['abilities']:
    ...
    if a['id']=='forgot-clothes': detail='Пассивно · без расхода'
    left_entries.append({'id':a['id'],'name':a['name'],'detail':detail,'body':skill_bodies[a['id']],
                         'icon':UI/'skill-icons/golovach-lena'/f"{a['id']}.png"})

right_entries=[]
# Match entry lengths across rows to preserve legible type and aligned dividers.
for iid in ['fire-fists','dragonborn-armour','buldak-noodles','camera-pendant','sleep-scroll']:
    icon=UI/'item-icons/golovach-lena'/f'{iid}.png'
    detail=meta(actions[iid]) if iid in ['buldak-noodles','sleep-scroll'] else ('Обычная атака · без расхода' if iid=='fire-fists' else 'С мастером · без расхода' if iid=='camera-pendant' else 'Пассивно · без расхода')
    name=actions[iid]['name'] if iid=='buldak-noodles' else items[iid]['name']
...
for left,right in zip(left_entries,right_entries,strict=True):
    ...
    y+=height
footer_y=y+10
footer_end=para('<b>С мастером · Огненная кровь:</b> +2 урона против существ холода; входящий холодный урон +2.',
...
c.showPage();c.save()
report={'card':'golovach-lena','campaign':'penisuela','rules':'Penisuela, selected by user','pages':1,'bodyFont':10.5,
        ...
        'abilityIds':list(skill_bodies),'itemIds':list(item_bodies),'itemsWithoutOriginalIcon':['fire-fists'],
