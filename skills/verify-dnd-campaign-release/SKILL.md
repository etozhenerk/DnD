...
Сохранить отдельный отчёт release verification: команды и результаты, проверенные маршруты и сценарии, размеры viewport, ошибки консоли, найденные дефекты, пути к скриншотам и непроверенные пункты. Зарегистрировать его отдельным артефактом `releaseVerification` в `workflow.json`. Не объявлять релиз готовым при блокирующей ошибке или неполной обязательной проверке.
