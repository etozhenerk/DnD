...
- для персонажа по реальному человеку — `realPersonReference` из общего контракта: режим источника, основа характера, уровень сходства, степень трансформации, сохраняемые и запрещённые признаки;
- статус `planned`.

## Правила

Не создавать декоративные арты без места использования. Для карты учитывать прозрачность и контур суши. Для частного человека требовать фотографию и согласованную степень расовой трансформации. Для публичной фигуры принимать точное имя и выбранный публичный образ; фотографию запрашивать только для конкретного периода или внешности.

Не записывать имя частного источника и путь к личной фотографии в `art-plan.json`. Указывать `referenceRequiredAtGeneration: true`, чтобы запросить фото повторно. Имя публичной фигуры допустимо в рабочем арт-плане, но не должно автоматически попадать в канонические данные и alt-текст.
